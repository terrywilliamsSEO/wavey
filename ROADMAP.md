...
Current next task: keep the passive phase-rule branch protected and keep the `central_hf_scattering_branch` firewalled. In `runs\release_phase_proof_pack_3d_20260619_234039`, the fixed quarter-dt `41^3` proof pack classified as `release_phase_quarter_dt_proof_supported`: proof-candidate cutoffs `17.94`, `17.9425`, and `17.945` preserved strict clean 9/8, with phase `0.5048-0.5094`. The requested one-step recalibrated resolution lift then ran at `51^3` in `runs\release_phase_resolution_lift_3d_20260620_091834` and classified as `release_phase_resolution_lift_failed`; the read-only postmortem in `runs\release_phase_resolution_postmortem_3d_20260620_100043` classified as `resolution_lift_blurred_returns_no_predictive_recalibration`. The modal audit in `runs\release_phase_modal_audit_3d_20260620_110344` classified as `resolution_blur_mechanism_supported`, and the dispersion audit in `runs\release_phase_dispersion_audit_3d_20260620_150931` classified as `scalable_blur_model_supported`: `41^3` and `51^3` preserve the same dominant shell-energy band while `51^3` collapses strict counts to 7/6 with looser returns still present. The spatial phase instrumentation reproduction in `runs\spatial_phase_instrumentation_3d_20260620_170518` classified as `spatial_phase_decoherence_supported`: the `51^3` failed-lift candidate loses shell/radial/angular phase coherence relative to the `41^3` proof row, while return spread does not grow and radial center shift is small. The read-only phase-precompensation design in `runs\spatial_phase_precomp_design_3d_20260620_175852` classified as `no_safe_phase_correction`: 352 matched sector samples produced a low-dimensional model R2 of only `0.00531`, and per-return global phase-error std was `1.04098` radians, so no low-dimensional global/face/cubic/harmonic/release-phase correction is safe to try. The read-only source-spectrum design audit in `runs\source_spectrum_design_audit_3d_20260620_181010` classified as `source_spectrum_narrowing_candidate_supported`: the current continuous hard-cutoff source has far sideband fraction `0.049396`, while a same-cutoff same-frequency same-work smooth `sin^2` envelope reduces that to `0.000516` and lowers source bandwidth ratio to `0.301`. This supports only one future `51^3` smooth-envelope candidate plus two controls if explicitly run next; it does not authorize a sweep. The first central high-frequency burst ladder in `runs\central_burst_3d_20260620_103248` classified as `central_burst_transient`: the cleanest row and its half-dt check produced only 1/0 shell returns, while the 0.92 rows produced repeated counts only with shell exit and outer/shell around 6.85. Do not run a precompensated `51^3` candidate, the proposed old 51^3 retry, `61^3`, nearby cutoff tuning, central-burst broadening, or any source-shape sweep. The only supported remaining scale-lift physics path is a single smooth-envelope `51^3` candidate with two controls.
...
- Source-spectrum-design result: the current hard-cutoff source has mean far sideband fraction `0.049396`; a same-cutoff smooth `sin^2` envelope reduces that to `0.000516` while matching the work proxy with amplitude scale about `1.633`. The observed `51^3` modal bandwidth growth remains `0.290751`, strict major loss is `2`, spatial coherence drop is `0.147229`, and phase precompensation failed, so the audit supports one future smooth-envelope candidate plus two controls as the last plausible scale-lift route.

### In Progress

- One possible next physics step, only if explicitly requested: a single `51^3` smooth-envelope release-phase candidate plus two controls, using the candidate JSON from `runs\source_spectrum_design_audit_3d_20260620_181010`. Do not broaden it into a sweep.
...
- The five-cutoff blind confirmation, four-cutoff half-dt validation, fixed half-dt recentering map, quarter-dt proof pack, requested `51^3` resolution lift, read-only postmortem, modal audit, dispersion audit, spatial phase instrumentation reproduction, precompensation design, and source-spectrum design audit are complete. Treat the quarter-dt strict-clean proof span as a `41^3` result, not a scale-validated claim.
- The spatial phase instrumentation points to sector/radius phase decoherence at `51^3`, and the read-only precompensation design did not find a safe low-dimensional correction. The source-spectrum audit found one plausible non-spatial path: same-release temporal smoothing to narrow carrier sidebands. If run, it must be one `51^3` smooth-envelope candidate plus two controls, not cutoff tuning, frequency tuning, spatial phase masks, or source-shape sweeps.
...
- 2026-06-20: Source-spectrum result: the current hard-cutoff source window has nontrivial sidebands, and a same-release smooth `sin^2` envelope theoretically reduces far sideband fraction from `0.049396` to `0.000516` while keeping frequency, cutoff phase, and total work proxy fixed. This justifies at most one future smooth-envelope `51^3` candidate plus two controls.
