...
- No passive scale-lift physics step is currently justified. The allowed smooth-envelope candidate and the measured patch-level phase-conjugate mirror have both run and failed the joint count-plus-coherence gates, the read-only modal sparsity audit found the source-shaped `51^3` rows share the same reconstruction/participation signature, and the return-family gate audit did not support detector-only rescue; do not broaden these into a sweep.
