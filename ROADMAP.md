...
Current next task: keep the passive phase-rule branch protected and keep the `central_hf_scattering_branch` firewalled. In `runs\release_phase_proof_pack_3d_20260619_234039`, the fixed quarter-dt `41^3` proof pack classified as `release_phase_quarter_dt_proof_supported`: proof-candidate cutoffs `17.94`, `17.9425`, and `17.945` preserved strict clean 9/8, with phase `0.5048-0.5094`. The requested one-step recalibrated resolution lift then ran at `51^3` in `runs\release_phase_resolution_lift_3d_20260620_091834` and classified as `release_phase_resolution_lift_failed`; the read-only postmortem in `runs\release_phase_resolution_postmortem_3d_20260620_100043` classified as `resolution_lift_blurred_returns_no_predictive_recalibration`. The modal audit in `runs\release_phase_modal_audit_3d_20260620_110344` classified as `resolution_blur_mechanism_supported`, and the dispersion audit in `runs\release_phase_dispersion_audit_3d_20260620_150931` classified as `scalable_blur_model_supported`: `41^3` and `51^3` preserve the same dominant shell-energy band while `51^3` collapses strict counts to 7/6 with looser returns still present. The new spatial phase instrumentation reproduction in `runs\spatial_phase_instrumentation_3d_20260620_170518` classified as `spatial_phase_decoherence_supported`: the `51^3` failed-lift candidate loses shell/radial/angular phase coherence relative to the `41^3` proof row, while return spread does not grow and radial center shift is small. This points to sector/radius spatial phase decoherence rather than simple coherent widening or shell-window misalignment. The first central high-frequency burst ladder in `runs\central_burst_3d_20260620_103248` classified as `central_burst_transient`: the cleanest row and its half-dt check produced only 1/0 shell returns, while the 0.92 rows produced repeated counts only with shell exit and outer/shell around 6.85. Do not run a source-shaped `51^3` candidate unless a future mechanism gives a concrete phase-precompensation design; do not run the proposed 51^3 retry, auto-run `61^3`, tune nearby cutoffs, call central burst an improvement to the phase rule, or broaden central burst without a new mechanism-specific reason.
...
- None. The recalibrated `51^3` resolution lift, read-only postmortem, first central HF scattering ladder, read-only modal audit, read-only dispersion audit, and spatial phase instrumentation reproduction are complete.
...
- The five-cutoff blind confirmation, four-cutoff half-dt validation, fixed half-dt recentering map, quarter-dt proof pack, requested `51^3` resolution lift, read-only postmortem, modal audit, dispersion audit, and spatial phase instrumentation reproduction are complete. Treat the quarter-dt strict-clean proof span as a `41^3` result, not a scale-validated claim.
- The spatial phase instrumentation points to sector/radius phase decoherence at `51^3`; do not run a source-shaped `51^3` candidate until a mechanism-derived phase pre-compensation design is specified.
