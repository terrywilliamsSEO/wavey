...
- No passive scale-lift physics step is currently justified. The one allowed smooth-envelope candidate has run and failed the joint count-plus-coherence gates; do not broaden it into a sweep.
...
- The five-cutoff blind confirmation, four-cutoff half-dt validation, fixed half-dt recentering map, quarter-dt proof pack, requested `51^3` resolution lift, read-only postmortem, modal audit, dispersion audit, spatial phase instrumentation reproduction, precompensation design, source-spectrum design audit, and smooth-envelope `51^3` rescue test are complete. Treat the quarter-dt strict-clean proof span as a `41^3` result, not a scale-validated claim.
- The spatial phase instrumentation points to sector/radius phase decoherence at `51^3`, and the read-only precompensation design did not find a safe low-dimensional correction. The source-spectrum audit found a plausible non-spatial path, but the fixed smooth-envelope run failed the joint strict-count and coherence gates. Do not run cutoff tuning, frequency tuning, spatial phase masks, source-shape sweeps, or larger grids from this branch without a new mechanism.
