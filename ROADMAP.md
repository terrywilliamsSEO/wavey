...
Current next task: keep the passive phase-rule branch protected and keep the new `central_hf_scattering_branch` firewalled. In `runs\release_phase_proof_pack_3d_20260619_234039`, the fixed quarter-dt `41^3` proof pack classified as `release_phase_quarter_dt_proof_supported`: proof-candidate cutoffs `17.94`, `17.9425`, and `17.945` preserved strict clean 9/8, with phase `0.5048-0.5094`. The requested one-step recalibrated resolution lift then ran at `51^3` in `runs\release_phase_resolution_lift_3d_20260620_091834` and classified as `release_phase_resolution_lift_failed`; the read-only postmortem in `runs\release_phase_resolution_postmortem_3d_20260620_100043` classified as `resolution_lift_blurred_returns_no_predictive_recalibration`, so do not run the proposed 51^3 retry, auto-run `61^3`, or tune nearby cutoffs from this result. The first central high-frequency burst ladder in `runs\central_burst_3d_20260620_103248` classified as `central_burst_transient`: the cleanest row and its half-dt check produced only 1/0 shell returns, while the 0.92 rows produced repeated counts only with shell exit and outer/shell around 6.85. Do not call central burst an improvement to the phase rule, and do not expand it into a broad sweep without a new mechanism-specific reason.
...
- None. The recalibrated `51^3` resolution lift, read-only postmortem, and first central HF scattering ladder are complete.
