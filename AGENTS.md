...
- Do not run broad long sweeps or broad 3D sweeps; active second-pulse controls are shelved for now, and the current conservative interpretation is the narrow passive phase-lock cluster preserves or improves the clean 9/8 refocusing family under stricter detection. The read-only return-map predictor supports a phase rule centered near 0.50 cycles and recommends only five future blind-confirmation cutoffs if physics is requested: 17.932885, 17.937885, 17.9225, 17.965, and 17.915. The first passive boundary-inner-edge resonator layer did not widen or stabilize the cluster, so do not expand resonator variants without a new mechanism-specific reason. Do not add traps, rotation, medium shaping, defects, grid changes, frequency combinations, active reinjection, or broad sweeps.
...
- The current next physics step, if requested, should be only the five-cutoff blind confirmation recommended by the return-map predictor. Keep 41^3, neutral lattice, stronger sponge, inner-sponge-edge source, matched primary work per physical source area, frequency 0.92, and radius-5 shell metrics fixed. Do not expand the passive resonator layer unless a new mechanism explains why a different passive coupling should avoid the observed strict-count degradation.
