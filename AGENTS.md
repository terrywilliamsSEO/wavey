...
- Do not run broad long sweeps or larger 3D grids; the next step is to stay at 31^3 and narrow around the preserved six-face cubic 3D boundary geometry.
...
- The 3D source-geometry control classified as `boundary_source_geometry_preserves_near_shell`: six-face cubic remained the cleanest boundary case, while uniform/reduced-face/random boundary variants were global-outer-window flagged and direct core/shell forcing was transient rather than retained.
- The current next physics step is a tiny 31^3 narrowing control around the six-face cubic boundary geometry before increasing grid size.
