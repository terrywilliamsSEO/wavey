...
- The read-only source-spectrum design audit in `runs\source_spectrum_design_audit_3d_20260620_181010` classified as `source_spectrum_narrowing_candidate_supported`: hard-cutoff source bandwidth was `0.106874`, smooth-envelope source bandwidth was `0.03218`, and far sideband fraction dropped from `0.049396` to `0.000516`. The candidate JSON is marked `recommended=true`, but it is only a theory gate for one future `51^3` smooth-envelope candidate plus two controls.
...
- The current next physics step is not another cutoff tuning pass; the blind confirmation, half-dt numerical validation, fixed half-dt recentering map, quarter-dt proof pack, requested `51^3` resolution lift, and postmortem are complete. Keep the frozen candidate card fixed for interpretation at `41^3`: neutral lattice, stronger sponge, inner-sponge-edge sign-flip cubic boundary source, matched primary work per physical source area, frequency 0.92, radius-5 shell metrics, no active second pulses, and no resonator layer. Do not expand the passive resonator layer unless a new mechanism explains why a different passive coupling should avoid the observed strict-count degradation. Do not run the proposed recalibrated `51^3` retry, `61^3`, or nearby `51^3` phase tuning automatically; a future scale step needs an explicit new mechanism first.
