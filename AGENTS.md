...
- Do not run broad long sweeps or broad 3D sweeps; active second-pulse controls are shelved for now, and the current conservative interpretation is the narrow passive phase-lock cluster preserves or improves the clean 9/8 refocusing family under stricter detection. The read-only return-map predictor supported a phase rule centered near 0.50 cycles, and the fixed five-cutoff blind confirmation classified as `release_phase_blind_confirmed`. The baseline/half-dt numerical validation classified as `release_phase_dt_sensitive`, then the fixed half-dt recentering map classified as `release_phase_half_dt_recentered`: strict 9/8 shifted upward to cutoffs `17.9375-17.945`, phase `0.5025-0.5094`. Do not tune nearby cutoffs or run quarter dt automatically after seeing those results. The first passive boundary-inner-edge resonator layer did not widen or stabilize the cluster, so do not expand resonator variants without a new mechanism-specific reason. Do not add traps, rotation, medium shaping, defects, grid changes beyond explicit numerical validation, frequency combinations, active reinjection, or broad sweeps.
...
- The current next physics step is not another cutoff tuning pass; the blind confirmation, half-dt numerical validation, and fixed half-dt recentering map are complete. Keep 41^3, neutral lattice, stronger sponge, inner-sponge-edge source, matched primary work per physical source area, frequency 0.92, and radius-5 shell metrics fixed for any future mechanism-specific check. Do not expand the passive resonator layer unless a new mechanism explains why a different passive coupling should avoid the observed strict-count degradation. Do not run quarter dt unless explicitly requested as a targeted check of the recentered window.
