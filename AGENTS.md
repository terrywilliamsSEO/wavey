...
- Do not run broad long sweeps or broad 3D sweeps; active second-pulse controls are shelved for now, and the current conservative interpretation is the narrow passive phase-lock cluster preserves or improves the clean 9/8 refocusing family under stricter detection. The read-only return-map predictor supported a phase rule centered near 0.50 cycles, and the fixed five-cutoff blind confirmation classified as `release_phase_blind_confirmed`. Do not tune nearby cutoffs after seeing that result. The first passive boundary-inner-edge resonator layer did not widen or stabilize the cluster, so do not expand resonator variants without a new mechanism-specific reason. Do not add traps, rotation, medium shaping, defects, grid changes, frequency combinations, active reinjection, or broad sweeps.
