...
- Do not run broad long sweeps or broad 3D sweeps; active second-pulse controls are shelved for now, and the next step is one tighter passive cutoff-only check around cutoff 17.94 at fixed frequency 0.92, not traps, rotation, medium shaping, defects, grid changes, frequency combinations, active reinjection, or broad sweeps.
...
- The current next physics step is a tighter passive cutoff-only check around cutoff 17.94 to see whether the 11/10 result has neighboring support or is a single-point artifact. Keep 41^3, neutral lattice, stronger sponge, inner-sponge-edge source, matched primary work per physical source area, frequency 0.92, and radius-5 shell metrics fixed.
