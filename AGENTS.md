...
- The current next physics step is not another cutoff tuning pass; the blind confirmation, half-dt numerical validation, fixed half-dt recentering map, quarter-dt proof pack, and requested `51^3` resolution lift are complete. Keep the frozen candidate card fixed for interpretation at `41^3`: neutral lattice, stronger sponge, inner-sponge-edge sign-flip cubic boundary source, matched primary work per physical source area, frequency 0.92, radius-5 shell metrics, no active second pulses, and no resonator layer. Do not expand the passive resonator layer unless a new mechanism explains why a different passive coupling should avoid the observed strict-count degradation. Do not run `61^3` or nearby `51^3` phase tuning automatically; a future scale step needs an explicit resolution-equivalence or failure-mode rationale first.
