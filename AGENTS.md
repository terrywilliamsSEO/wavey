...
- Do not run broad long sweeps or broad 3D sweeps; active second-pulse controls are shelved for now, and the current conservative interpretation is the narrow passive phase-lock cluster preserves or improves the clean 9/8 refocusing family under stricter detection. The read-only return-map predictor supported a phase rule centered near 0.50 cycles, and the fixed five-cutoff blind confirmation classified as `release_phase_blind_confirmed`. The follow-up baseline/half-dt numerical validation classified as `release_phase_dt_sensitive`: half dt preserved strict 9/8 at `17.937885` but not `17.932885`, so do not claim the two-row strong pocket is half-dt invariant. Do not tune nearby cutoffs after seeing those results. The first passive boundary-inner-edge resonator layer did not widen or stabilize the cluster, so do not expand resonator variants without a new mechanism-specific reason. Do not add traps, rotation, medium shaping, defects, grid changes beyond explicit numerical validation, frequency combinations, active reinjection, or broad sweeps.
...
- The current next physics step is not another cutoff tuning pass; the blind confirmation and half-dt numerical validation are complete. Keep 41^3, neutral lattice, stronger sponge, inner-sponge-edge source, matched primary work per physical source area, frequency 0.92, and radius-5 shell metrics fixed for any future mechanism-specific check. Do not expand the passive resonator layer unless a new mechanism explains why a different passive coupling should avoid the observed strict-count degradation.
