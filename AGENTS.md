...
- Do not run broad long sweeps or broad 3D sweeps; active second-pulse controls are shelved for now, and the next step is an event/threshold robustness audit around the narrow passive phase-lock needle at cutoff 17.93-17.94 and fixed frequency 0.92, not traps, rotation, medium shaping, defects, grid changes, frequency combinations, active reinjection, or broad sweeps.
...
- The current next physics step is event/threshold robustness around the narrow 17.93-17.94 passive phase-lock needle before claiming robust 11/10 physics. Keep 41^3, neutral lattice, stronger sponge, inner-sponge-edge source, matched primary work per physical source area, frequency 0.92, and radius-5 shell metrics fixed.
