...
- Do not run broad long sweeps or broad 3D sweeps; the next step is a second tiny 41^3 local refinement around the clean refocusing winners, especially `cutoff_long` and `frequency_high`.
...
- The current next physics step is a second tiny local refocusing refinement around the winning cutoff/frequency pocket. Vary only cutoff near 18, frequency near 0.94, and possibly a small combined cutoff/frequency adjustment while preserving 41^3, neutral lattice, stronger sponge, inner-sponge-edge source, matched work per physical source area, and radius-5 shell metrics. Do not widen into a grid, source-geometry sweep, or more defect-parameter fishing.
