...
- The current next physics step is not another cutoff tuning pass; the blind confirmation, half-dt numerical validation, fixed half-dt recentering map, and quarter-dt proof pack are complete. Keep the frozen candidate card fixed for interpretation: 41^3, neutral lattice, stronger sponge, inner-sponge-edge sign-flip cubic boundary source, matched primary work per physical source area, frequency 0.92, radius-5 shell metrics, no active second pulses, and no resonator layer. Do not expand the passive resonator layer unless a new mechanism explains why a different passive coupling should avoid the observed strict-count degradation. Only after the proof pack, a future scale check should be one release-phase-recalibrated 51^3 or 61^3 candidate plus two controls, not a sweep.
