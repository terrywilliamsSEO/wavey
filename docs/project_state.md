...
- The `prototype-3d-isochronous-anchor-cleanup-control` command is available but not yet run. It is a fixed `41^3` cleanup follow-up testing whether smooth-taper compensation variants can keep the `isochronous_anchor_0p5x` memory/strict/comb gains while reducing off-comb back near neutral. It must not tune cutoff, frequency, source, `51^3`, `61^3`, active pulses, resonators, or source shaping.
