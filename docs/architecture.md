...
- `prototype-3d-cutoff-phase-map-control`: tiny 41^3 release-phase timing map around cutoff 18, with compact phase-offset and sign-flip/polarity families, a passive island-refinement preset, ranked cutoff-phase output, and release-phase island stability checks.
...
- `simulation/prototype_3d_cutoff_phase_map.py`: tiny calibrated 41^3 release-phase timing map. It keeps frequency fixed, varies cutoff near 18, records phase at cutoff, compares small global phase-offset perturbations, includes a compact sign-flip/polarity family, supports a passive island-refinement preset around `sign_flip_cutoff_minus_0p1`, and emits ranked results by major shell-window peaks, refocus peaks, no exit, retention, outer/shell below 1.0, decay closest to zero, and global outer false. Reports include a release-phase island stability section that checks whether top rows are neighboring cutoff samples rather than a single isolated point.
...
- Use `prototype-3d-cutoff-phase-map-control` after the cutoff-frequency map fails additivity; it tests whether release phase/cutoff timing and polarity family control refocusing. The `--release-phase-island-refinement` preset is the current passive follow-up around the supported sign-flip timing family.
