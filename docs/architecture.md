...
- Use `prototype-3d-release-phase-dt-recenter` only as the fixed half-dt recentering map after the dt-sensitive validation. It has no user-facing cutoff sweep knob. The first run classified as `release_phase_half_dt_recentered`, with strict 9/8 preserved across cutoffs `17.9375-17.945`; do not run quarter dt automatically.
