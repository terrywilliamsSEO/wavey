...
- `prototype-3d-cutoff-phase-map-control`: tiny 41^3 release-phase timing map around cutoff 18, with compact phase-offset and sign-flip/polarity families, passive island-refinement and phase-lock needle presets, ranked cutoff-phase output, release-phase island stability checks, phase-lock needle-width reporting, and event-threshold sensitivity auditing.
...
- `simulation/prototype_3d_cutoff_phase_map.py`: tiny calibrated 41^3 release-phase timing map. It keeps frequency fixed, varies cutoff near 18, records phase at cutoff, compares small global phase-offset perturbations, includes a compact sign-flip/polarity family, supports passive island-refinement and sign-flip-only phase-lock needle presets, and emits ranked results by major shell-window peaks, refocus peaks, no exit, retention, outer/shell below 1.0, decay closest to zero, and global outer false. Reports include release-phase island stability, phase-lock needle-width, and event-threshold sensitivity sections.
