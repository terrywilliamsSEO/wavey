...
- `prototype-3d-second-pulse-control`: tiny 41^3 active second-pulse control from the best release phase, with no-pulse and passive-extension work-normalized comparators.
...
- Use `prototype-3d-second-pulse-control` after the release-phase timing island is established; it tests whether active reinjection beats no-pulse and passive-extension baselines after total injected work is accounted for.
